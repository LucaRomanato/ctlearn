...
def conv_block(inputs, training, params, reuse=None):
    with tf.variable_scope("Basic_conv_block", reuse=reuse):
        ...
        x = inputs
        #x = tf.layers.batch_normalization(x, momentum=bn_momentum, training=training)
...
def fc_block_after_cnn(inputs, params, reuse=None):
    with tf.variable_scope("Basic_fc_block_after_cnn", reuse=reuse):

        units_list = [layer['units'] for layer in
                      params['basic']['fc_block_after_cnn']['layers']]
        numbers_list = [layer.get('number', 1) for layer in
                        params['basic']['fc_block_after_cnn']['layers']]
        x = inputs
...
def fc_block(inputs, params, reuse=None, name = basic_block):
